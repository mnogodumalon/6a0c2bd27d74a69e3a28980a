...
export function BuchungenDialog({ open, onClose, onSubmit, defaultValues, kurseList, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: BuchungenDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'buchungen');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="kurs" entity="Kurse">\n${JSON.stringify(kurseList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      ...
      const schema = `{\n  "kurs": string | null, // Display name from Kurse (see <available-records>)\n  "buchungsart": LookupValue | null, // Buchungsart (select one key: "einzel" | "karte_10" | "abo_monat" | "abo_jahr") mapping: einzel=Einzelbuchung, karte_10=10er-Karte, abo_monat=Monatsabo, abo_jahr=Jahresabo\n  "anzahl": number | null, // Anzahl\n  "buchungsdatum": string | null, // YYYY-MM-DD\n  "preis": number | null, // Preis (EUR)\n  "bezahlt": boolean | null, // Bezahlt\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["kurs", "mitglied"]);
        ...
        if (kursName) {
          const kursMatch = kurseList.find(r => matchName(kursName!, [String(r.fields.kursname ?? '')]));
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="einzel">Einzelbuchung</SelectItem>
                <SelectItem value="karte_10">10er-Karte</SelectItem>
                <SelectItem value="abo_monat">Monatsabo</SelectItem>
                <SelectItem value="abo_jahr">Jahresabo</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              type="number"
              value={fields.anzahl ?? ''}
              onChange={e => setFields(f => ({ ...f, anzahl: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={extractRecordId(fields.mitglied) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, mitglied: v === 'none' ? undefined : createRecordUrl(APP_IDS.MITGLIEDER, v) }))}
            >
              <SelectTrigger id="mitglied"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {mitgliederList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>

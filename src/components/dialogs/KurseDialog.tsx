...
export function KurseDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: KurseDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kursname": string | null, // Kursname\n  "einzelpreis": number | null, // Einzelpreis (EUR)\n  "karten10_preis": number | null, // 10er-Karten-Preis (EUR)\n  "monatsabo_preis": number | null, // Monatsabo-Preis (EUR)\n}`;
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="karten10_preis">10er-Karten-Preis (EUR)</Label>
            <Input
              id="karten10_preis"
              type="number"
              value={fields.karten10_preis ?? ''}
              onChange={e => setFields(f => ({ ...f, karten10_preis: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="monatsabo_preis">Monatsabo-Preis (EUR)</Label>
            <Input
              id="monatsabo_preis"
              type="number"
              value={fields.monatsabo_preis ?? ''}
              onChange={e => setFields(f => ({ ...f, monatsabo_preis: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <DialogFooter>
